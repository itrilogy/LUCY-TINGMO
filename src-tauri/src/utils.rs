...
pub fn cancel_current_operation(app: &AppHandle) {
    ...
    // Close the lightweight RecordingSession first so mode/phase logs stay coherent.
    if let Some(hub) = app.try_state::<Arc<crate::session::SessionHub>>() {
        let _ = hub.cancel_active();
    }
    ...
    audio_manager.segment_pipeline().cancel();
