use crate::audio_toolkit::{
    apply_custom_words, filter_transcription_output, join_segment_texts, plan_ranges_for_max_ms,
...
impl SessionResultEvent {
    #[allow(dead_code)]
    pub fn ok(text: String) -> Self {
        Self::ok_with_raw(text, None)
    }

    pub fn ok_with_raw(text: String, raw_text: Option<String>) -> Self {
        let raw_text = raw_text.filter(|r| {
            let rt = r.trim();
            !rt.is_empty() && rt != text.trim()
        });
        Self {
            text,
    ...
    pub fn empty(message: impl Into<String>) -> Self {
        Self {
            text: String::new(),
            status: SessionResultStatus::Empty,
...
impl TranscriptionManager {
    ...
    /// Transcribe with control over unload and optional decode context (segment chaining).
    pub fn transcribe_with(
        &self,
        audio: Vec<f32>,
        call_opts: TranscribeCallOptions,
    ) -> Result<String> {
        ...
        let result = {
            ...
            let mut capability_max_audio_ms: Option<i64> = None;
            if let LoadedEngine::TranscribeCpp(session) = &engine {
                ...
                capability_max_audio_ms = Some(caps.max_audio_ms);
                debug!(
            ...
            // Long audio must be split to the model's safe window. SenseVoice
            // (and similar) degrade badly past ~30s — see install-build logs
            // "beyond the ~30 s window" and 代码研究/05-问题分析-长录音.md.
            let max_audio_ms =
                resolve_max_audio_ms(&model_arch, &active_model, capability_max_audio_ms);
            let ranges = plan_ranges_for_max_ms(&audio, max_audio_ms);
            if ranges.len() > 1 {
                let audio_secs = audio_len as f64 / 16_000.0;
                info!(
                    "session_id=batch mode=batch_auto_split event=auto_split audio_secs={:.2} segments={} max_audio_ms={} model='{}'",
                    audio_secs,
                    ranges.len(),
                    max_audio_ms,
                    active_model
            ...
            let app_for_progress = self.app_handle.clone();
            let multi_segment = ranges.len() > 1;
            let total_segments = ranges.len() as u32;
            let outer_context = call_opts.context_prompt.clone();
            let transcribe_result = catch_unwind(AssertUnwindSafe(|| -> Result<String> {
                let mut segment_texts: Vec<String> = Vec::with_capacity(ranges.len());
                let mut prev_context = outer_context;
                for (idx, (start, end)) in ranges.iter().copied().enumerate() {
                    let segment = &audio[start..end];
                    let seg_secs = segment.len() as f64 / 16_000.0;
                    if multi_segment {
                        debug!(
                    ...
                    let text = transcribe_audio_slice(
                        &mut engine,
                        segment,
                        &settings,
                        &validated_language,
                    ...
                    if multi_segment {
                        debug!(
                            "Segment {}/{} produced {} chars",
                            idx + 1,
                            ranges.len(),
                            text.chars().count()
                        );
                    }
                    // Chain decode context for the next tile (whisper initial_prompt).
                    if !text.trim().is_empty() {
                        prev_context = Some(text.clone());
                    }
                    segment_texts.push(text);
                    if multi_segment {
                        let cumulative = join_segment_texts(&segment_texts);
                        ...
                        // Reuse the live text channel so the expanded overlay
                        // can grow the transcript as segments complete.
                        let _ = StreamTextEvent {
                ...
                Ok(join_segment_texts(&segment_texts))
            }));
        ...
        info!(
            "Transcription completed in {:.2}s for {:.2}s of audio ({:.2}x real-time){} — result_chars={}",
            elapsed_secs,
            audio_secs,
            speedup,
            translation_note,
...
/// Resolve the safe per-segment audio window (ms) for batch auto-split.
///
/// Prefer live GGUF [`Capabilities::max_audio_ms`] when advertised (> 0).
/// Otherwise fall back to family heuristics / [`DEFAULT_MAX_AUDIO_MS`] (30s).
fn resolve_max_audio_ms(
    model_arch: &str,
    model_id: &str,
    capability_max_audio_ms: Option<i64>,
) -> u64 {
    let arch = model_arch.to_ascii_lowercase();
    let id = model_id.to_ascii_lowercase();

    // Qwen3-ASR (and similar LLM-decoder ASR) has a hard *generation* budget of
    // 256 tokens in transcribe-cpp (`k_max_new`). Capability `max_audio_ms` is
    // only the *input* context bound (~tens of minutes) and must NOT drive
    // segment length — a 2‑min Chinese slice easily exceeds 256 tokens and the
    // run returns OUTPUT_TRUNCATED (history re-transcribe then fails).
    // Keep tiles short so each decode can finish within the token budget.
    // See install logs: "qwen3_asr run: output truncated at 256 tokens".
    if arch.contains("qwen3") || id.contains("qwen3-asr") || id.contains("qwen3_asr") {
        return 20_000; // 20s — conservative for dense ZH speech under 256 tokens
    }

    if let Some(ms) = capability_max_audio_ms {
        if ms > 0 {
            // Keep splits practical for interactive dictation pipelines.
            return (ms as u64).clamp(5_000, 120_000);
        }
        // 0 = "no practical limit" in transcribe-cpp; still chunk large files
        // for memory and quality on long meetings (5 min tiles).
        if ms == 0 {
            return 300_000;
        }
    }
    // SenseVoice is the known hard ~30s case from install-build logs.
    if arch.contains("sensevoice") || id.contains("sensevoice") {
        return DEFAULT_MAX_AUDIO_MS;
    }
    // Whisper-family also operates on ~30s tiles internally; keep the same cap
    // so application-level splits stay predictable.
    if arch == "whisper" || id.contains("whisper") {
        return DEFAULT_MAX_AUDIO_MS;
    }
    // Conservative default for unknown / other GGUF & ONNX engines.
    DEFAULT_MAX_AUDIO_MS
}

/// Options for a single transcription call.
#[derive(Clone, Debug, Default)]
pub struct TranscribeCallOptions {
    /// When false, skip idle unload so multi-segment pipelines keep the engine warm.
    pub unload_after: bool,
    /// Optional decode context (previous segment text / custom words seed).
    pub context_prompt: Option<String>,
}

/// Run one engine pass on a single PCM slice (already within the safe window).
fn transcribe_audio_slice(
    ...
    match engine {
        LoadedEngine::TranscribeCpp(session) => {
            // Custom words + optional prior-segment context become the initial
            // prompt ONLY for whisper-family models.
            let family = if !model_is_whisper {
                ...
                if let Some(ctx) = context_prompt.map(str::trim).filter(|s| !s.is_empty()) {
                    // Cap context so the prompt stays short for decode quality.
                    let clipped: String = ctx.chars().rev().take(200).collect::<String>()
                        .chars()
                        .rev()
                        .collect();
                    parts.push(clipped);
                ...
                if parts.is_empty() {
                    ...
                    Some(RunExtension::Whisper(WhisperRunOptions {
                        initial_prompt: Some(parts.join(" ")),
                        ..Default::default()
                    }))
                }
            };
            ...
            debug!(
                "transcribe-cpp run: task={:?}, language={:?}, initial_prompt={}, samples={}",
                run_options.task,
            ...
            match session.run(audio, &run_options) {
                Ok(t) => Ok(t.text),
                // Qwen3/Cohere-style decoders hit a fixed generation budget
                // (256 tokens). Prefer the partial transcript over aborting the
                // whole multi-segment job (history re-transcribe used to fail).
                Err(transcribe_cpp::Error::OutputTruncated { message, partial }) => {
                    let partial_text = partial
                        .as_ref()
                        .map(|t| t.text.clone())
                        .unwrap_or_default();
                    warn!(
                        "transcribe-cpp output truncated ({}): keeping {} partial chars; \
                         consider shorter segments for this model",
                        message,
                        partial_text.chars().count()
                    );
                    if partial_text.trim().is_empty() {
                        Err(anyhow::anyhow!(
                            "transcribe-cpp output truncated with empty partial: {}",
                            message
                        ))
                    } else {
                        Ok(partial_text)
                ...
                Err(e) => Err(anyhow::anyhow!("transcribe-cpp transcription failed: {}", e)),
            }
        }
...
mod tests {
    ...
    #[test]
    fn resolve_max_audio_ms_defaults_to_30s() {
        assert_eq!(
            resolve_max_audio_ms("sensevoice", "handy-computer/SenseVoiceSmall-gguf", None),
            DEFAULT_MAX_AUDIO_MS
        );
        assert_eq!(
            resolve_max_audio_ms("whisper", "large-v3", None),
            DEFAULT_MAX_AUDIO_MS
        );
        assert_eq!(
            resolve_max_audio_ms("", "unknown-model", None),
            DEFAULT_MAX_AUDIO_MS
        );
        assert_eq!(
            resolve_max_audio_ms("sensevoice", "x", Some(30_000)),
            30_000
        );
        assert_eq!(
            resolve_max_audio_ms("any", "x", Some(0)),
            300_000,
            "0 means no practical limit — use 5min tiles"
        );
        assert_eq!(
            resolve_max_audio_ms("any", "x", Some(1_000)),
            5_000,
            "clamp floor"
        );
        // Qwen3: ignore huge capability max_audio_ms; short tiles for 256-token gen budget.
        assert_eq!(
            resolve_max_audio_ms(
                "qwen3_asr",
        ...
        assert_eq!(
            resolve_max_audio_ms("", "Qwen3-ASR-0.6B-Q8_0.gguf", Some(120_000)),
            20_000
        );
    }
