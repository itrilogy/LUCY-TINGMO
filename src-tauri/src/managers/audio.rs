...
use crate::managers::segment_pipeline::SegmentPipeline;
...
fn create_audio_recorder(
    ...
    segment_pipeline: Arc<SegmentPipeline>,
    ...
    // Segment callback feeds the semi-realtime pipeline when it is active.
    let recorder = AudioRecorder::new()
        ...
        .with_segment_callback({
            let pipeline = segment_pipeline;
            move |seg| {
                if pipeline.is_active() {
                    pipeline.push_segment(seg.pcm, seg.silence_ms, seg.force_cut);
                }
            }
        });
...
impl AudioRecordingManager {
    ...
    pub fn new(
        ...
        let manager = Self {
            ...
            stream_router,
            segment_pipeline: Arc::new(SegmentPipeline::new()),
    ...
    pub fn preload_vad(&self) -> Result<(), anyhow::Error> {
        ...
        if recorder_opt.is_none() {
            ...
            *recorder_opt = Some(create_audio_recorder(
                ...
                Arc::clone(&self.segment_pipeline),
            )?);
        }
        Ok(())
    }
    ...
    pub fn segment_pipeline(&self) -> Arc<SegmentPipeline> {
        Arc::clone(&self.segment_pipeline)
    }
