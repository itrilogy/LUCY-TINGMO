...
impl HistoryManager {
    ...
    /// Rename a history entry's display title (user-editable).
    pub fn update_title(&self, id: i64, title: String) -> Result<HistoryEntry> {
        let title = title.trim().to_string();
        if title.is_empty() {
            return Err(anyhow!("Title cannot be empty"));
        }
        // Keep titles compact in the list header.
