...
/// Index of the quietest short window in `[from, to)` (cut at window mid).
fn find_quiet_cut_index(audio: &[f32], from: usize, to: usize) -> Option<usize> {
    if to <= from || to > audio.len() {
        return None;
    }
    let window = SEGMENT_ENERGY_WINDOW.max(1);
    if to - from < window {
        return Some(from);
    }
    let step = (window / 2).max(1);
    let mut best_mid = from;
    ...
    while start + window <= to {
        ...
        if energy < best_energy {
            best_energy = energy;
            best_mid = start + window / 2;
        }
        start += step;
    }
    Some(best_mid.clamp(from, to))
}
...
/// Finished speech segment for semi-realtime ASR.
#[derive(Debug, Clone)]
pub struct FinishedSpeechSegment {
    pub pcm: Vec<f32>,
    /// Estimated trailing silence that closed this segment (ms).
    pub silence_ms: u64,
    /// True when closed by max-length force-cut rather than VAD pause.
    pub force_cut: bool,
}

/// Callback when a speech segment ends (VAD hangover finished or max length hit).
/// Used for semi-realtime segmented ASR.
pub type SegmentCallback = Arc<dyn Fn(FinishedSpeechSegment) + Send + Sync + 'static>;

/// Soft max for one live VAD segment (~28s @ 16 kHz) before a forced cut.
/// Slightly under the common ~30s model window so force-cuts + carry-over stay safe.
const SEGMENT_MAX_SAMPLES: usize = 16_000 * 28;
/// Samples carried into the next segment after a force-cut (boundary phonemes).
const SEGMENT_FORCE_OVERLAP_SAMPLES: usize = 16_000 * 4 / 10; // 400 ms
/// Search the last this many samples of a max window for a quieter cut.
const SEGMENT_BOUNDARY_SEARCH_SAMPLES: usize = 16_000 * 2; // 2 s
/// Never force-cut before this length (keeps tiny fragments out of the ASR queue).
const SEGMENT_MIN_FORCE_SAMPLES: usize = 16_000 * 5; // 5 s
/// Drop segments shorter than this (~0.5s) as noise/click.
const SEGMENT_MIN_SAMPLES: usize = 16_000 / 2;
/// RMS probe window for quiet-cut search (~100 ms).
const SEGMENT_ENERGY_WINDOW: usize = 16_000 / 10;

struct SegmentCapture {
    buf: Vec<f32>,
    in_speech: bool,
    /// Consecutive non-speech frames while buffering (for pause-duration punct).
...
impl AudioRecorder {
    ...
    /// Register a callback for completed speech segments (VAD end-of-utterance
    /// or max-length force cut). Only active when VAD is enabled for the session.
    pub fn with_segment_callback<F>(mut self, cb: F) -> Self
    where
        F: Fn(FinishedSpeechSegment) + Send + Sync + 'static,
    {
...
fn run_consumer(
    ...
    let mut segment_capture = SegmentCapture::new();
    ...
    fn flush_segment(
        ...
        if capture.buf.is_empty() {
            capture.in_speech = false;
            capture.trailing_silence_frames = 0;
            return;
        }
        // Smoothed VAD only emits Noise after hangover expires, so a normal
        // close has ~hangover_frames * 30ms of silence. We also keep a frame
        // counter for diagnostics / future multi-frame Noise accumulation.
        let silence_ms = if force_cut {
            0
        } else {
            let from_frames = (capture.trailing_silence_frames as u64).saturating_mul(30);
            // Offline hangover default is 15 frames (450ms); use at least that.
            from_frames.max(450)
        };
        let seg = std::mem::take(&mut capture.buf);
        capture.in_speech = false;
        capture.trailing_silence_frames = 0;
        if seg.len() < SEGMENT_MIN_SAMPLES {
            return;
        }
        if let Some(cb) = segment_cb {
            cb(FinishedSpeechSegment {
                pcm: seg,
                silence_ms,
                force_cut,
            });
    ...
    /// Prefer a quieter sample near the soft max so continuous speech is not
    /// hard-sliced mid-word; keep a short overlap for the next segment.
    fn force_cut_if_needed(capture: &mut SegmentCapture, segment_cb: &Option<SegmentCallback>) {
        while capture.buf.len() >= SEGMENT_MAX_SAMPLES {
            let hard_end = SEGMENT_MAX_SAMPLES.min(capture.buf.len());
            let earliest = SEGMENT_MIN_FORCE_SAMPLES.min(hard_end).max(1);
            let search_from = hard_end
                .saturating_sub(SEGMENT_BOUNDARY_SEARCH_SAMPLES)
                .max(earliest);
            let cut = find_quiet_cut_index(&capture.buf, search_from, hard_end)
                .unwrap_or(hard_end)
                .clamp(earliest, hard_end);
            // Guarantee progress even if energy search returns a degenerate point.
            let cut = if cut == 0 { hard_end } else { cut };

            let seg: Vec<f32> = capture.buf[..cut].to_vec();
            let rest_start = cut.saturating_sub(SEGMENT_FORCE_OVERLAP_SAMPLES);
            capture.buf = capture.buf[rest_start..].to_vec();
            capture.in_speech = !capture.buf.is_empty();

            log::debug!(
                "VAD segment force-cut: flushed={:.2}s remain={:.2}s cut_at={:.2}s (overlap={:.2}s)",
                seg.len() as f64 / 16_000.0,
                capture.buf.len() as f64 / 16_000.0,
                cut as f64 / 16_000.0,
                (cut - rest_start) as f64 / 16_000.0
            );
            ...
            // Pathological: no progress (should not happen with earliest >= 1).
            if capture.buf.len() >= SEGMENT_MAX_SAMPLES && cut <= rest_start {
                let hard = capture.buf.split_off(SEGMENT_MAX_SAMPLES.min(capture.buf.len()));
                let forced = std::mem::replace(&mut capture.buf, hard);
                if forced.len() >= SEGMENT_MIN_SAMPLES {
                    if let Some(cb) = segment_cb {
                        cb(FinishedSpeechSegment {
                            pcm: forced,
                            silence_ms: 0,
    ...
    fn handle_frame(
        ...
        audio_cb: &Option<AudioFrameCallback>,
        segment_cb: &Option<SegmentCallback>,
        segment_capture: &mut SegmentCapture,
        out_buf: &mut Vec<f32>,
    ) {
        if !recording {
            return;
        ...
        if vad_policy == VadPolicy::Disabled {
            emit(samples);
            // Without VAD, force-cut energy-aware windows for semi-realtime.
            if segment_cb.is_some() {
                segment_capture.buf.extend_from_slice(samples);
                segment_capture.in_speech = true;
                segment_capture.trailing_silence_frames = 0;
                force_cut_if_needed(segment_capture, segment_cb);
            }
            return;
        }
        ...
        if let Some(cfg) = vad {
            ...
            match det.push_frame(samples).unwrap_or(VadFrame::Speech(samples)) {
                VadFrame::Speech(buf) => {
                    emit(buf);
                    if segment_cb.is_some() {
                        segment_capture.buf.extend_from_slice(buf);
                        segment_capture.in_speech = true;
                        segment_capture.trailing_silence_frames = 0;
                        // Long continuous speech: soft force-cut near quiet boundary.
                        force_cut_if_needed(segment_capture, segment_cb);
                    }
                }
                VadFrame::Noise => {
                    // Hangover frames still arrive as Noise once speech ended.
                    if segment_capture.in_speech || !segment_capture.buf.is_empty() {
                        segment_capture.trailing_silence_frames =
                            segment_capture.trailing_silence_frames.saturating_add(1);
                        // End of utterance (after hangover): flush segment for live ASR.
                        // Smoothed VAD emits continuous Noise after hangover expires;
                        // we flush on the first Noise once hangover path closed speech.
                        flush_segment(segment_capture, segment_cb, false);
                    }
                }
            }
        } else {
        ...
        while let Ok(cmd) = cmd_rx.try_recv() {
            match cmd {
                ...
                Cmd::Stop(reply_tx) => {
                    ...
                    // Flush any open speech segment for live ASR before returning PCM.
                    flush_segment(&mut segment_capture, &segment_cb, false);
