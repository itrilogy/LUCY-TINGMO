...
pub fn add_post_process_prompt(
    ...
    settings::write_settings(&app, settings);
    let _ = crate::prompts_fs::sync_prompt_file(&app, &new_prompt);
...
pub fn update_post_process_prompt(
        ...
        .find(|p| p.id == id)
    {
        ...
        let snapshot = existing_prompt.clone();
        settings::write_settings(&app, settings);
        let _ = crate::prompts_fs::sync_prompt_file(&app, &snapshot);
        Ok(())
    } else {
...
pub fn delete_post_process_prompt(app: AppHandle, id: String) -> Result<(), String> {
    ...
    settings::write_settings(&app, settings);
    let _ = crate::prompts_fs::remove_prompt_file(&app, &id);
    Ok(())
}
...
pub fn open_prompts_folder(app: AppHandle) -> Result<(), String> {
    crate::prompts_fs::open_prompts_folder(&app)
}

#[tauri::command]
#[specta::specta]
pub fn export_prompts_to_folder(app: AppHandle) -> Result<String, String> {
    crate::prompts_fs::export_all_prompts(&app)
}
...
pub fn change_transcription_mode_setting(
    ...
    let mut settings = settings::get_settings(&app);
    let mut mode = mode;
    if mode == settings::TranscriptionMode::NativeStream {
        let supports = app
            ...
            .unwrap_or(false);
        if !supports {
            // Enforce: no native realtime without streaming capability.
            mode = settings::TranscriptionMode::VadSegmented;
            log::info!(
                "NativeStream rejected for model '{}'; using vad_segmented",
                settings.selected_model
            );
            settings.transcription_mode = mode;
            settings::write_settings(&app, settings);
            return Err(
                "CURRENT_MODEL_NO_STREAMING".to_string(), // frontend maps to i18n + toast
            );
        }
    }
    settings.transcription_mode = mode;
...
#[tauri::command]
#[specta::specta]
pub fn change_asr_markers_rule_enabled_setting(
    app: AppHandle,
    enabled: bool,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.asr_markers_rule_enabled = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_asr_markers_inject_glossary_setting(
    app: AppHandle,
    enabled: bool,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.asr_markers_inject_glossary = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
...
#[tauri::command]
#[specta::specta]
pub fn change_asr_markers_auto_accept_new_setting(
    app: AppHandle,
    enabled: bool,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.asr_markers_auto_accept_new = enabled;
    settings::write_settings(&app, settings);
    Ok(())
}
