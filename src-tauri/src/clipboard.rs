...
/// Write `text` to the system clipboard (platform-aware).
...
/// Pastes text using the clipboard: writes text, sends paste keystroke.
///
/// When `restore_previous` is true (legacy auto-paste path), saves and restores
/// the prior clipboard after the keystroke. When false (explicit "paste to front"),
/// leaves the transcription on the clipboard so the paste cannot race with a
/// restore of stale content.
fn paste_via_clipboard(
    ...
    let clipboard = app_handle.clipboard();
    let saved_text = if restore_previous {
    ...
    let saved_image = if restore_previous && saved_text.is_none() {
        clipboard.read_image().ok().map(|image| image.to_owned())
    } else {
        None
    };

    // Always put the intended text on the clipboard before the paste keystroke.
    write_clipboard_text(app_handle, text)?;

    // Best-effort verify on platforms where clipboard write can lag (helps diagnose
    // and gives the pasteboard a moment to settle when content was already present).
    let verify = clipboard.read_text().unwrap_or_default();
    if verify != text {
        info!(
            "Clipboard verify mismatch after write (got {} chars, expected {}); re-writing",
            verify.chars().count(),
            text.chars().count()
        );
        write_clipboard_text(app_handle, text)?;
    ...
    // Floor delays so a restore (or the keystroke) cannot beat a slow pasteboard.
    let delay_before = paste_delay_ms.max(80);
    let delay_after = paste_delay_after_ms.max(if restore_previous { 120 } else { 80 });
    std::thread::sleep(Duration::from_millis(delay_before));
    ...
    std::thread::sleep(Duration::from_millis(delay_after));

    if !restore_previous {
        // Explicit paste-to-front: keep transcription on the clipboard.
        // Re-assert after the keystroke in case the OS swapped the pasteboard.
        let _ = write_clipboard_text(app_handle, text);
        return Ok(());
    }
...
/// Legacy paste path (may restore prior clipboard depending on settings).
/// Kept for potential auto-paste re-enable; overlay uses [`paste_to_frontmost`].
#[allow(dead_code)]
pub fn paste(text: String, app_handle: AppHandle) -> Result<(), String> {
    paste_inner(text, app_handle, /* explicit_front */ false)
}

/// Explicit "paste to frontmost" path (overlay button).
///
/// Always puts the transcription on the clipboard first, then pastes into the
/// frontmost app, and **leaves** the transcription on the clipboard (does not
/// restore prior clipboard content). This avoids the race where a restore of
/// stale clipboard content wins over the paste keystroke when the board already
/// held text.
pub fn paste_to_frontmost(text: String, app_handle: AppHandle) -> Result<(), String> {
    paste_inner(text, app_handle, /* explicit_front */ true)
}

fn paste_inner(text: String, app_handle: AppHandle, explicit_front: bool) -> Result<(), String> {
    let settings = get_settings(&app_handle);
    let mut paste_method = settings.paste_method;
    let paste_delay_ms = settings.paste_delay_ms;
    let paste_delay_after_ms = settings.paste_delay_after_ms;

    // Explicit front paste must actually paste — never no-op on PasteMethod::None.
    if explicit_front && paste_method == PasteMethod::None {
        info!("PasteMethod::None overridden to CtrlV for explicit paste-to-front");
    ...
    info!(
        "Using paste method: {:?}, delay before: {}ms, delay after: {}ms, explicit_front: {}",
        paste_method, paste_delay_ms, paste_delay_after_ms, explicit_front
    );
    ...
    match paste_method {
        ...
        PasteMethod::Direct => {
            // Still seed the clipboard on explicit front so users can re-paste.
            if explicit_front {
                let _ = write_clipboard_text(&app_handle, &text);
            }
            paste_direct(
        ...
        PasteMethod::CtrlV | PasteMethod::CtrlShiftV | PasteMethod::ShiftInsert => {
            // Legacy/auto path restores prior clipboard; explicit front keeps text.
            let restore_previous = !explicit_front
    ...
    // After pasting, optionally copy to clipboard based on settings (legacy path).
    // Explicit front already left text on the clipboard.
    if !explicit_front && settings.clipboard_handling == ClipboardHandling::CopyToClipboard {
        write_clipboard_text(&app_handle, &text)?;
    }
