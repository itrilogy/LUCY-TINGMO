...
pub fn switch_active_model(app: &AppHandle, model_id: &str) -> Result<(), String> {
    ...
    // Native realtime requires streaming; otherwise force VAD semi-realtime.
    let mut mode_fallback = false;
    if settings.transcription_mode == crate::settings::TranscriptionMode::NativeStream
        && !model_info.supports_streaming
    {
        settings.transcription_mode = crate::settings::TranscriptionMode::VadSegmented;
        mode_fallback = true;
        log::info!(
            "Model '{}' has no native streaming; transcription_mode → vad_segmented",
            model_id
        );
    }
    ...
    if mode_fallback {
        let _ = app.emit("transcription-mode-fallback", "vad_segmented");
    }
