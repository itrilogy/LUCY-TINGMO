...
// Live (streaming) and result share the same native window footprint so the
// in-recording "展开" card can grow to the same outer size as the post-record
// result panel without clipping. CSS keeps the unexpanded pill smaller inside
// this transparent stage.
// Card is --ov-result-w (400) + border; height ≈ text 140 + chrome + slack.
const OVERLAY_PANEL_WIDTH: f64 = 420.0;
const OVERLAY_PANEL_HEIGHT: f64 = 220.0;

/// Overlay window size (logical) for a given UI state.
fn overlay_dimensions(state: &str) -> (f64, f64) {
    match state {
        "streaming" | "result" => (OVERLAY_PANEL_WIDTH, OVERLAY_PANEL_HEIGHT),
...
fn show_overlay_state(app_handle: &AppHandle, state: &str) {
    show_overlay_state_with_force(app_handle, state, false);
}

/// Like [`show_overlay_state`], but `force` shows even when overlay_style is None.
/// Used for the post-transcription result panel (no auto-paste — user must see text).
fn show_overlay_state_with_force(app_handle: &AppHandle, state: &str, force: bool) {
    // Whether the overlay shows at all is governed by overlay_style; position
    // only chooses Top vs Bottom placement. Result panels may force-show.
    let settings = settings::get_settings(app_handle);
    if !force && settings.overlay_style == OverlayStyle::None {
        return;
    }
...
/// Force-show the expanded streaming-sized overlay (progress during long batch).
/// Used when auto-splitting so users see segment progress even if Minimal was set.
pub fn show_streaming_overlay_forced(app_handle: &AppHandle) {
    show_overlay_state_with_force(app_handle, "streaming", true);
}

/// Compact “transcribing” pill (legacy / optional UI path).
/// Live long-audio and VAD paths use streaming + result overlays instead.
#[allow(dead_code)]
pub fn show_transcribing_overlay(app_handle: &AppHandle) {
    show_overlay_state(app_handle, "transcribing");
}

/// Final transcript panel: expanded overlay with copy (never auto-hides paste).
/// Always shown when there is text — product decision: no auto-paste.
pub fn show_result_overlay(app_handle: &AppHandle) {
    show_overlay_state_with_force(app_handle, "result", true);
}

/// Compact “processing / polishing” pill (legacy / optional UI path).
#[allow(dead_code)]
...
pub fn update_overlay_position(app_handle: &AppHandle) {
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            // Note: result panel also uses large size; flag is set when showing.
            let (width, height) = overlay_dimensions(state);
            if let Err(error) = place_windows_overlay(app_handle, &overlay_window, width, height) {
...
mod tests {
    ...
    fn windows_overlay_bounds_support_negative_monitor_origins() {
        // Panel footprint matches result / live-expanded (420×220 logical).
        assert_eq!(
            windows_overlay_bounds(
            ...
            // width/height: 420*1.25=525, 220*1.25=275
            // x: (-2560 + (2560-525)/2).round() = -1542.5 → -1543 (half away from 0)
            (-1543, 915, 525, 275)
        );
    }
}
